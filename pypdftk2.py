...
This is a remate of the pypdftk Python module to drive the awesome pdftk binary
available at:
...
def fill_form_v2(pdf_path, datas={}, temp_file=None, out_file=None, flatten=True):
    '''
        Fills a PDF form with given dict input data.
        Return temp file if no out_file provided.
    '''
    # cleanOnFail = False
    gen_xfdf_v2(temp_file, datas)
    # handle = None
    if not out_file:
        # cleanOnFail = True
        handle, out_file = tempfile.mkstemp()

    cmd = "%s %s fill_form %s output %s" % (PDFTK_PATH, pdf_path, temp_file, out_file)
    if flatten:
        cmd += ' flatten'
    try:
        run_command(cmd, True)
...
def gen_xfdf_v2(temp_file, datas={}):
    ''' Generates a temp XFDF file suited for fill_form function, based on dict input data '''
    fields = []
    for key, value in datas.items():
        fields.append("""        <field name="%s"><value>%s</value></field>""" % (key, value))
